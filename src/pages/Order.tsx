...
import { FaUpload, FaExclamationTriangle, FaComments, FaFileAlt, FaHistory, FaCheckCircle, FaClock, FaPaperPlane } from "react-icons/fa";
...
function Order() {
  ...
  const { data: taskResponse, isLoading: isTaskLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await axiosPrivate.get(`/task/${orderResponse?.data?.gigId}`);
      return response.data;
    },
  ...
  const orderDetails = orderResponse?.data && taskResponse?.data ? {
    _id: orderResponse.data._id,
    status: orderResponse.data.status,
    deliveryDate: new Date(orderResponse.data.deadline).getTime(),
    orderNumber: orderResponse.data.initialTransactionHash?.slice(0, 10) || "N/A",
    orderDate: new Date(orderResponse.data.createdAt).toLocaleDateString(),
    ...
    activities: [
      { type: "message", content: "Project started", time: "2 hours ago", user: "System" },
      { type: "file", content: "Initial requirements uploaded", time: "1 day ago", user: "Client" },
    ],
    milestones: [
      { 
  ...
  const { mutate: deliverOrder, isLoading: isDelivering } = useMutation({
    ...
    onError: (error: any) => {
      ...
      toast({
        ...
        description: error.response?.data?.msg || "There was an error completing the order. Please try again.",
        variant: "destructive",
        className: "bg-white border border-gray-200",
        style: {
          backgroundColor: "white",
  ...
  const { mutate: acceptDelivery, isLoading: isAccepting } = useMutation({
    mutationFn: async () => {
      ...
      const response = await axiosPrivate.post(`/order/${orderId}/orderDelivered`);
    ...
    onError: (error: any) => {
      ...
      toast({
        ...
        description: error.response?.data?.msg || "There was an error accepting the delivery. Please try again.",
        variant: "destructive",
        className: "bg-white border border-gray-200",
        style: {
          backgroundColor: "white",
              ...
              <div className="col-span-2 space-y-6">
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      <div className="flex items-center gap-4 mb-4">
                        ...
                        `}>
                          {orderDetails.status === 'completed' ? 'Completed' : 'In Progress'}
                        </Badge>
                    ...
                    <div className="text-right">
                      <Typography variant="2xl" className="font-bold text-green-700">
                  ...
                  <Tabs defaultValue="activity" className="w-full">
                    <div className="border-b">
                      <TabsList className="flex p-0 bg-transparent">
                        {["activity", "details", "deliverables", "messages"].map((tab) => (
                          <TabsTrigger
                            key={tab}
                            value={tab}
                            ...
                            {orderDetails.activities.map((activity, index) => (
                              <div key={index} className="flex items-start gap-4">
                                <div className="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center text-gray-600">
                                  {activity.type === 'message' && <FaComments />}
                          ...
                          <div className="bg-gray-50 border-2 border-dashed rounded-xl p-8">
                            ...
                            <label htmlFor="file-upload" className="cursor-pointer block text-center">
                              <div className="space-y-4">
                                ...
                                <div>
                                  <Typography variant="lg" className="font-medium text-gray-900">
                                    Upload Deliverables
                                  </Typography>
                ...
                <div className="bg-white rounded-xl p-6 shadow-sm">
                  ...
                  <div className="prose max-w-none" 
                       dangerouslySetInnerHTML={{ __html: orderDetails.description }} 
                  />
                </div>
              </div>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <p className="font-medium text-gray-900">Delivery Deadline</p>
                  </div>
                ...
                <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
                  ...
                  <div className="space-y-3 text-sm">
                    <div className="flex justify-between items-center py-2 border-b">
                      ...
                      <span className="font-medium">{orderDetails.seller.username}</span>
                    </div>
                    <div className="flex justify-between items-center py-2 border-b">
                      <span className="text-gray-600">Email</span>
                      <span className="font-medium">{orderDetails.seller.email}</span>
                    </div>
                    <div className="flex justify-between items-center py-2">
                      <span className="text-gray-600">Wallet</span>
                      <span className="font-medium truncate" title={orderDetails.seller.wallet}>
                        {orderDetails.seller.wallet.slice(0, 6)}...{orderDetails.seller.wallet.slice(-4)}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
                  <div className="space-y-4">
                    ...
                    <p className="text-sm text-gray-600">
                      If you're experiencing any issues with this order, our support team is here to help resolve them.
                    </p>
                ...
                <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
                  ...
                  <div className="space-y-3 text-sm">
                    <div className="flex justify-between items-center py-2 border-b">
                      ...
                      <span className="font-medium truncate" title={orderResponse.data.initialTransactionHash}>
                    ...
                    <div className="flex justify-between items-center py-2 border-b">
                      ...
                      <span className="font-medium">{orderDetails.revision}</span>
                    </div>
                    <div className="flex justify-between items-center py-2">
